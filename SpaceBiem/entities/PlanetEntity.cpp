...
namespace spacebiem
{
    PlanetEntity::PlanetEntity(float x, float y, Color color, float w, float h, const string& texture, const string& borderTexture, int pScoreBonus, const string& pName)
    {
        ...
        addComponent("text", new TextComponent(to_string(pScoreBonus), { 255,255,255, 50 }, w / 2.f, h / 2.f + 20.f, false, true));
        addComponent("text", new TextComponent(pName, { 255,255,255,255 }, w / 2.f, h / 2.f - 10.f, false, true, "name"));
